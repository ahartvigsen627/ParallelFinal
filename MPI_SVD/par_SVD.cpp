...
//SVD - MPI and OPENMP IMPLEMENTATION -  BLOCK DISTRIBUTION
  ...
  //#pragma omp parallel num_threads(numberOfThreads)
   ...
   while(conv > epsilon){ 		//convergence
	   ...
	   for(int i=1; i<M; i++){ 		//convergence
		   ...
		   myend = mystart+psize;
		   myend = myend > i ? i : myend;

		   alpha = 0.0;
		   //local_a = 0.0
		   		  
		   //#pragma omp parallel for //private(beta, gamma, zeta, t) //reduction(+:alpha)
			for(int j=mystart; j<myend; j++){    //0; j<i; j++){			  
			  ...
			  //#pragma omp parallel for num_threads(numberOfThreads) reduction(+:alpha) reduction(+:beta) reduction(+:gamma)
			  for(int k = 0; k<N; k++){		//my_rank; k<M; k+=comm_sz){
				  /*				  
				  if(j==0)
					  local_a += (U_t[i][k] * U_t[i][k]);
				  //if(i==1)
				  local_b += tempu[k]*tempu[k];//(U_t[j][k] * U_t[j][k]);
				  local_g += (U_t[i][k] * tempu[k]);//U_t[j][k]);
				  */
				  if(j==mystart)
			  ...
			  /*
				...
				*/
				...
				local_c[j-mystart] = 1.0 / (sqrt (1.0 + (t*t)));				//extract cos
				local_s[j-mystart] = local_c[j-mystart]*t;
			...
			//}
			//MPI_Scatter(U_t, N, MPI_DOUBLE, 
	   }	  
	   //if((conv < epsilon) && (extra_it == 0))
		//{
			//conv = 1.0;
			//extra_it++;
		//}
